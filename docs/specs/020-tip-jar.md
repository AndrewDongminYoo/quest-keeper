...
- **Verify the tip rows against the App Store Connect products, not the `.storekit` file.** `QuestKeeper.storekit` is committed at the repo root and its reference lives only in the per-user scheme (git-ignored via `xcuserdata/**/xcschemes/*.xcscheme`), so a fresh clone must re-point the scheme's Run options at it. On the `xcodebuild` CLI path that reference had no effect at all — neither `LaunchAction` nor `TestAction`, neither scheme — and every render came from the real ASC products instead. Xcode's own Run may honour it; that is untested.
  New Swift files need no `project.pbxproj` edit: `PBXFileSystemSynchronizedRootGroup` covers `QuestKeeper`, `QuestKeeperShared`, `QuestKeeperTests`, and `QuestKeeperUITests`.
- **Assert the rows, do not read a screenshot.** Give each row an `accessibilityIdentifier` and assert both the count and that the labels are distinct. A capture nearly got misread twice — once as a missing row of unknown cause, once as a blank screen that was only an unfinished layout.
