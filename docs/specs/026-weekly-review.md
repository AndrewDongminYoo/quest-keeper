...
- **No quest history.** A fresh installation has no week to review, and the sentence it would show is nonsense on a board that has never held a quest. History is the union of the append-only `quest_created` fact and the current store contents: the fact alone misses a user migrated from the pre-measurement schema, whose quests survived with no events, and the store alone misses a user who has since deleted every quest.
